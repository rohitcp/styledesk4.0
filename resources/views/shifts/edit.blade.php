...
@section('title', __('shifts.edit_title'))
  ...
  <main class="w-full px-4 sm:px-5 lg:px-6 pt-5 sm:pt-6 pb-[200px]">
    <div class="styledesk_form">
      ...
      <nav class="text-[13px] text-sub" aria-label="Breadcrumb">
        ...
        <span class="text-ink">{{ $shift->staff?->displayName() }} · {{ $shift->date->translatedFormat('j M Y') }}</span>
      </nav>

      <div class="mt-3 flex flex-wrap items-start gap-4">
        <div class="min-w-0 flex-1">
          <h1 class="text-[24px] sm:text-[28px] font-bold text-head tracking-tight">{{ __('shifts.edit_title') }}</h1>
        </div>
      ...
      <form id="shiftForm" method="POST" action="{{ route('shifts.update', $shift) }}" class="mt-6 space-y-5">
        @csrf
        @method('PATCH')
        ...
        <div class="flex flex-wrap items-center gap-3">
          <button type="submit" data-submit-once data-busy-label="{{ __('shifts.saving') }}"
